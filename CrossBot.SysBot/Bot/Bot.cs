...
namespace CrossBot.SysBot
{
    ...
    public sealed class Bot : SwitchRoutineExecutor<BotConfig>
    {
        ...
        protected override async Task MainLoop(CancellationToken token)
        {
            ...
            // Auto get offset
            Offset = await this.GetCurrentPlayerOffset((uint)OffsetHelper.InventoryOffset, (uint)OffsetHelper.PlayerSize, token).ConfigureAwait(false);
        ...
        private async Task CleanUp(int count, CancellationToken token)
        {
            ...
            // Give us one free inventory space
